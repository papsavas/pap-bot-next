...
export class MonitoredCollection<K, V> extends Collection<K, V> {
	...
	get(key: K, trigger?: boolean) {
		if (trigger && this.monitors.get) this.monitors.get(this, key);
		return super.get(key);
	}

	set(key: K, value: V, trigger?: boolean) {
		if (trigger && this.monitors.set) this.monitors.set(key, value);
		return super.set(key, value);
	}

	delete(key: K, trigger?: boolean) {
		if (trigger && this.monitors.delete) this.monitors.delete(this, key);
