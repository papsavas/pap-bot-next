...
describe('Commands', async () => {
	...
	it.todo('Data should be synced', async (ctx) => {
		...
		const registeredGlobalCommands = (await rest.get(
			Routes.applicationCommands(process.env.DISCORD_DEV_BOT_ID!)
		...
		for (const localData of localCommandData.values()) {
			const registeredData = registeredGlobalCommands.find(
				(c) => c.name === localData.name
			);
