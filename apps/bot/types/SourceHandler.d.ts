...
export type EditOptions<T> = T extends CommandInteraction
	? InteractionEditReplyOptions
	: T extends Message
	? MessageEditOptions
	: never;

export type ReplyOptions<T> = T extends CommandInteraction
	? InteractionReplyOptions
	: T extends Message
	? MessageReplyOptions
	: never;

export type SourceHandlerOptions = <T extends CommandSource>(
	source: T
) => {
	source: T;
	deferReply: (
