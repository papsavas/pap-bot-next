...
export async function migrateMessages(
	...
	if (!(oldChannel.isTextBased() && newChannel.isTextBased()))
		throw new Error('Channels are not text based');
	...
	const fetchMessages = async (beforeMsgId?: string) => {
		const res = await oldChannel.messages.fetch({
			limit: MAX_LIMIT_MESSAGE_CHUNK,
			before: beforeMsgId,
		});
		return res.reverse();
	};
	...
	for (const [msdId, oldMessage] of allMessages) {
		...
		try {
			...
			const newAPIMessage = await webhookClient.send({
				...
				files: oldMessage.attachments.map((a) =>
					AttachmentBuilder.from(a as JSONEncodable<AttachmentPayload>).setName(
						a.name
					)
				),
			});
