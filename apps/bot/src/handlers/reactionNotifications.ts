...
export const createReactionNotificationsId = ({
	...
	userId,
	targetId,
}: ReactionNotifier) => createCombinedId(guildId, userId, targetId);

export const resolveReactionNotificationsId = (
	id: string
): ReactionNotifier => {
...
export const handleReactionNotifications = async (
	...
	if (reaction.message.inGuild()) {
		...
		if (shouldNotify) {
			console.log(
				`notifying ${msg.author.tag} for ${reactor.tag} reaction on ${msg.url}`
			);
			const messageValue =
				msg.content.length > 0 ? msg.content.slice(0, 1000) : `Jump to Message`;
			msg.author
				.send({
					embeds: [
						new EmbedBuilder({
							author: {
								name: reactor.tag,
								iconURL: reactor.avatarURL() ?? undefined,
							},
