...
const messageCreate = makeEvent({
	...
	async execute(message) {
		if (message.inGuild()) {
			...
			if (!guildCommandPrefix)
				throw new NotServedError('messageCreate', message.guildId);
			if (message.content.startsWith(guildCommandPrefix.prefix)) {
				const { primaryCommand } = sliceCommand(
					message,
					guildCommandPrefix.prefix
				);
				const cmd = ctx.commands.find(
					(cmd) => cmd.name === primaryCommand.toLowerCase()
				);
