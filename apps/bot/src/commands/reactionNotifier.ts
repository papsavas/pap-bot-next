...
const reactionNotifierCommand = new Command({
	...
	execute: async ({
		...
		if (!command.inGuild()) return reply({ content: warnings.only.guild });
		...
		return deferReply(
			async () => {
				const [subCmd, targetId] = [
					...
					(
						command.options.resolved?.roles ?? command.options.resolved?.members
					)?.firstKey(),
				];
				...
				switch (subCmd) {
					case addSubCmd: {
						...
						if (!targetId)
							return reply({
								content: `Please provide a target (role or member)`,
							});
						try {
							await user.send(
								italic(
									`This message establishes our DM channel where i will notify you about reaction notifications.`
								)
							);
						} catch (error: any) {
							if (
						...
						//update cache and db
						ctx.reactionNotifier.set(
							createReactionNotificationsId({ guildId, userId, targetId }),
							{
								userId,
								targetId,
								guildId,
						...
						//respond to user
						return reply({
							content: `Enabled notifications for ${targetId ?? 'all'}`,
						});
					}

					case showSubCmd: {
						const filtered = ctx.reactionNotifier.filter(
							(v) => v.guildId === guildId && v.userId === user.id
						);
						return reply({
							embeds: [
								new EmbedBuilder({
									...
									description:
										filtered.size === 0
											...
											: filtered.reduce(
													(acc, { targetId }) =>
														(acc += `${
															command.guild!.roles.cache.has(targetId)
																? roleMention(targetId)
					...
					case clearSubCmd: {
						const removed = ctx.reactionNotifier.sweep(
							(v) => v.guildId === guildId,
							true
						);
