...
const pollCommand = new Command({
	...
	execute: async ({ source, deferReply, reply, warnings, sliced, user }) => {
		if (!source.inGuild()) return reply({ content: warnings.only.guild });
		...
		return deferReply(async () => {
			const [text, timeLimit, pingRole, username] =
				source instanceof ChatInputCommandInteraction
					...
					: [sliced!.args1, null, null, user.username];

			const embed = new EmbedBuilder({
				title: `${username} started a poll`,
				description: text,
				footer: timeLimit
					? { text: `Lasts for ${timeLimit} minutes` }
					: undefined,
			});
			...
			const collector = response.createMessageComponentCollector({
				...
				filter: (button, collector) =>
					...
					collector.every(
						(collectedButton) => collectedButton.user.id !== button.user.id
					),
			});
			...
			collector.on('collect', (buttonInteraction) => {
				...
				response
					.edit({
						components: [
							new ActionRowBuilder<ButtonBuilder>().setComponents(
								ButtonBuilder.from(upvoteBtn).setLabel(upCount.toString()),
								ButtonBuilder.from(downvoteBtn).setLabel(downCount.toString())
							),
						],
					})
					.then(() =>
